# TODOs were implemented on 9.12.2021 by
...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            m = self.pi._distribution(state)

            action = m.sample()
            value_function = self.v(state) # equivalent to self.v.forward(state)
            log_prob = self.pi._log_prob_from_distribution(m, action)

            return action, value_function, log_prob
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        # TD residual -> performs worse!
        #self.phi_buf[path_slice] = deltas
        ...
        gammas = []
        gamma_i = 1
        for i in range(self.ret_buf[path_slice].shape[0]):
            gammas.append(gamma_i)
            gamma_i *= self.gamma
        gammas = np.array(gammas)
        
        self.ret_buf[path_slice] = discount_cumsum(rews, self.gamma)[:-1]* gammas
...
class Agent:
    ...
    def v_update(self, data):
        ...
        # In each update, compute a loss for the value function, call loss.backward() and 
        ...
        for i in range(100):
            ...
            obs_values = self.ac.v(obs)

            action = torch.argmax(self.ac.pi(obs)[0].probs, dim=1)

            val_target = ret + 0.99*action
            loss = criterion(obs_values, val_target)
            loss.backward()
    ...
    def get_action(self, obs):
        ...
        action, _, _ = self.ac.step(torch.tensor(obs).float())
        return action #np.random.choice([0, 1, 2, 3])
