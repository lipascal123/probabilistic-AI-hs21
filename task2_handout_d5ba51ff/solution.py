...
def run_solution(dataset_train: torch.utils.data.Dataset, data_dir: str = os.curdir, output_dir: str = '/results/', device='cpu') -> 'Model':
    """
...
class Model(object):
    ...
    def __init__(self):
        ...
        learning_rate = 2e-3  # training learning rates
        ...
        if use_densenet:
            ...
            self.network = BayesNet(in_features=28 * 28, hidden_features=hidden_layers, out_features=10).to(device)
    ...
    def bayesloss(self, output, batch_y, network):
        ce_loss = torch.nn.CrossEntropyLoss(size_average=False)
        
        feat, prior, posterior = output

        loss = 200*ce_loss(feat, batch_y)
        # print('a:', loss.item())

        j = len(posterior) - 1

        # for j in range(len(posterior)):
        #     print(200*ce_loss(feat, batch_y), 0.5*torch.sum(posterior[j] - prior[j])*1.0/(posterior[j].shape[0]*posterior[j].shape[1]))
        #     loss += 0.5*torch.sum(posterior[j] - prior[j])*1.0/(posterior[j].shape[0]*posterior[j].shape[1])


        for layer in network.layers:
            # print(200*ce_loss(feat, batch_y), 0.5 * (2 * torch.log(F.softplus(layer.weight_rho)/ torch.tensor(1.0)) - 1 + (torch.tensor(1.0) / F.softplus(layer.weight_rho)).pow(2) + ((layer.weight_mu) / F.softplus(layer.weight_rho)).pow(2)).sum())
            loss += 0.04 * (2 * torch.log(F.softplus(layer.weight_rho)/ torch.tensor(1.0)) - 1 + (torch.tensor(1.0) / F.softplus(layer.weight_rho)).pow(2) + ((layer.weight_mu) / F.softplus(layer.weight_rho)).pow(2)).sum()

        nllloss = F.nll_loss(F.log_softmax(feat, dim=1), batch_y, reduction='sum')


        return loss# + 700*nllloss
        # return 100*loss + 1*loss2# + 700*nllloss
        #return 1*loss/0.4 + 0*loss2/21.3 + 1*nllloss/49.9
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    loss = self.bayesloss(output, batch_y, self.network)
                    loss.backward()
                ...
                if batch_idx % self.print_interval == 0:
                    if isinstance(self.network, DenseNet):
                        ...
                        # print(current_logits.argmax(axis=1)[0:20])
                        # print(batch_y[0:20])
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        # sigma1 = 0.1
        # sigma2 = 0.001
        #self.prior =  MultivariateDiagonalGaussian(torch.zeros(out_features, in_features), torch.ones(out_features, in_features))
        ...
        #  )
        self.weight_mu = nn.Parameter(torch.Tensor(out_features, in_features).uniform_(-0.5, 0.5))
        self.weight_rho = nn.Parameter(torch.Tensor(out_features, in_features).uniform_(-3.5, -3)) # those values must be negative, to have a small sigma
        ...
        if self.use_bias:
            ...
            self.bias_mu = nn.Parameter(torch.Tensor(out_features).uniform_(-0.5, 0.5))
            self.bias_rho = nn.Parameter(torch.Tensor(out_features).uniform_(-3.5, -3)) # those values must be negative, to have a small sigma
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        weights = self.weights_var_posterior.sample()     # NOT VARIFIED

        # print("weights:", weights)
        log_prior = self.prior.log_likelihood(weights)    # NOT VARIFIED

        log_variational_posterior = self.weights_var_posterior.log_likelihood(weights) # NOT VARIFIED


        if self.use_bias: bias = self.bias_var_posterior.sample()
        else: bias = torch.zeros(self.out_features)

        feat = F.linear(inputs, weights, bias)

        return feat, log_prior, log_variational_posterior
...
class BayesNet(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: typing.Tuple[int, ...], out_features: int):
        ...
        self.layers = nn.ModuleList([
            BayesianLayer(feature_sizes[idx], feature_sizes[idx + 1], bias=False)
    ...
    #     return current_features
        ...
        for idx, current_layer in enumerate(self.layers):
            ...
            current_features = new_features
            log_prior.append(log_prior_layer)
            log_variational_posterior.append(log_variational_posterior_layer)
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        sum_xi_mu = 0
        for i in range(n):
            sum_xi_mu += (values[i] - self.mu)**2

        result = -0.5*n*np.log(2*np.pi) - 0.5*n*np.log(self.sigma**2) - 0.5/(self.sigma**2)*sum_xi_mu
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        softplus = Softplus()
        const_add = torch.add(torch.zeros((self.rho).shape), -0.5*np.log(2*np.pi))
        return (const_add - 0.5*torch.log(softplus(self.rho)**2) - 0.5/(softplus(self.rho)**2)*(values - self.mu)**2)

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        softplus = Softplus()
        epsilon = Normal(0,1).sample(self.rho.size())
        return self.mu + softplus(self.rho) * epsilon
...
def main():
    ...
    raw_train_data = np.load(os.path.join(data_dir, '../task2_handout_d5ba51ff/train_data.npz'))
    ...
    # Run actual solution
    run_solution(dataset_train, data_dir=data_dir, output_dir=output_dir, device=device)
